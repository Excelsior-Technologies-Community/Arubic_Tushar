...
function Stars({ count }) {
  return (
    <div className="arb-wm-stars">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} className={`arb-wm-star ${i <= count ? "arb-wm-star--filled" : ""}`} viewBox="0 0 24 24">
    ...
    >
      <div className="arb-wm-card__img-wrap">
        ...
        <img
          ...
          onError={(e) => { e.target.src = "https://placehold.co/400x480?text=No+Image"; }}
        />
        {hovered && (
      ...
      <div className="arb-wm-card__info">
        ...
        <div className="arb-wm-card__price">
          <span className="arb-wm-card__price-now">${Number(product.price).toFixed(2)}</span>
          {product.old_price && (
            <span className="arb-wm-card__price-old">${Number(product.old_price).toFixed(2)}</span>
          )}
        </div>
        {viewMode === "list" && (
...
export default function Women() {
  ...
  const [filters, setFilters]       = useState({ categories: [], compositions: [], styles: [], properties: [] });
  ...
  }, [currentPage, selectedCat, selectedPrice, selectedComp, selectedStyle, selectedProp, sortBy]);
  ...
  useEffect(() => {
    ...
    fetch(`${API}/products?${buildQuery()}`)
      ...
      .catch(() => setError("Server se connect nahi ho pa raha. Backend check karo."))
  ...
  const toggleSideCat = (cat) =>
    setOpenSideCats((p) => p.includes(cat) ? p.filter((c) => c !== cat) : [...p, cat]);

  const clearAll = () => {
    setSelectedCat(null); setSelectedPrice(null);
    setSelectedComp(null); setSelectedStyle(null); setSelectedProp(null);
    resetPage();
  };

  const hasFilters = selectedCat || selectedPrice !== null ||
    selectedComp || selectedStyle || selectedProp;

  const goPage = (p) => { setCurrentPage(p); window.scrollTo(0, 0); };
      ...
      <div className="arb-container arb-wm-layout">
              ...
              {sidebarCats.map((cat) => (
                <li key={cat} className="arb-wm-sb-cat-item">
                  <div className="arb-wm-sb-cat-row">
                    ...
                    <button className="arb-wm-sb-cat-toggle" onClick={() => toggleSideCat(cat)}>
                    ...
                    <ul className="arb-wm-sb-sub">
                      <li>
                        <button
                          ...
                          onClick={() => { setSelectedCat(cat.toLowerCase()); resetPage(); }}
                        >
                          All {cat}
                        </button>
                      </li>
            ...
            {filters.categories.map((c) => (
              <label key={c.category} className="arb-wm-sb-check">
                <input
                  ...
                  onChange={() => { setSelectedCat(selectedCat === c.category ? null : c.category); resetPage(); }}
                />
            ...
            {PRICE_RANGES.map((r, i) => (
              <label key={i} className="arb-wm-sb-radio">
                <input
                  ...
                  onChange={() => { setSelectedPrice(selectedPrice === i ? null : i); resetPage(); }}
                />
                <span>{r.label}</span>
              </label>
            ))}
            ...
            {filters.compositions.map((c) => (
              <label key={c.composition} className="arb-wm-sb-check">
                <input
                  ...
                  onChange={() => { setSelectedComp(selectedComp === c.composition ? null : c.composition); resetPage(); }}
                />
                <span>{c.composition} ({c.count})</span>
              </label>
            ))}
            ...
            {filters.styles.map((s) => (
              <label key={s.style} className="arb-wm-sb-check">
                <input
                  ...
                  onChange={() => { setSelectedStyle(selectedStyle === s.style ? null : s.style); resetPage(); }}
                />
                <span>{s.style} ({s.count})</span>
              </label>
            ))}
            ...
            {filters.properties.map((p) => (
              <label key={p.property} className="arb-wm-sb-check">
                <input
                  ...
                  onChange={() => { setSelectedProp(selectedProp === p.property ? null : p.property); resetPage(); }}
                />
                <span>{p.property} ({p.count})</span>
              </label>
            ))}
          </div>
        </aside>
        ...
        <main className="arb-wm-main">
          ...
          <div className="arb-wm-banner">
            {/* <div className="arb-wm-banner__text">
              <p className="arb-wm-banner__sub">UP TO 70%</p>
              <h2 className="arb-wm-banner__title">WOMEN COLLECTION</h2>
              <Link to="/shop" className="arb-wm-banner__link">
                SHOP NOW <span className="arb-wm-banner__arrow">⟶</span>
              </Link>
            </div> */}
            <img
          ...
          <div className="arb-wm-toolbar">
            <div className="arb-wm-toolbar__left">
              ...
              >
                <svg viewBox="0 0 16 16" fill="currentColor">
                  <rect x="0" y="0" width="16" height="4"/><rect x="0" y="6" width="16" height="4"/>
          ...
          {/* Products Grid / List */}
          <div className={`arb-wm-products ${viewMode === "list" ? "arb-wm-products--list" : "arb-wm-products--grid"}`}>
            {loading
              ? Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)
              : products.length > 0
                ? products.map((p) => <ProductCard key={p.id} product={p} viewMode={viewMode} />)
                : !error && <p className="arb-wm-no-results">No products match the selected filters.</p>
            }
          </div>
          ...
          {!loading && pagination.total > 0 && (
            <div className="arb-wm-pagination">
              <span className="arb-wm-pagination__info">
                ...
                {Math.min(currentPage * 12, pagination.total)} of {pagination.total} item(s)
              </span>
